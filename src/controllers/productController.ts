...
import { uploadImage, deleteImage, extractPublicId } from '../utils/cloudinaryService';
...
export const getFilterData = catchAsync(async (req, res) => {
  ...
  const brandCounts = allFilteredProducts.reduce((acc, product) => {
  ...
  const specsWithCounts = allFilteredProducts.reduce((acc, product) => {
...
export const createProduct = catchAsync(async (req, res, next) => {
  const { name, brand, category, description, variants: variantsString } = req.body;

  if (!name || !brand || !category || !description || !variantsString || !req.files) {
  ...
  const processedVariants = await Promise.all(
    variants.map(async (variant, index) => {
      ...
      const uploadedImages = await Promise.all(
        variantImages.map(image => uploadImage('products/variants', image.buffer))
      );
  ...
  if (updateData.variants) {
    ...
    const updatedVariants = await Promise.all(
      parsedVariants.map(async (variant: any, index: number) => {
        ...
        if (variant.saleOff) {
          if (!variant.saleOff.percentage || !variant.saleOff.startDate || !variant.saleOff.endDate) {
            return next(new AppError('Missing required sale off fields', 400));
          }

          if (variant.saleOff.percentage < 0 || variant.saleOff.percentage > 100) {
