...
interface ICart extends Document {
  ...
  products: { product: mongoose.Types.ObjectId; variant: mongoose.Types.ObjectId; quantity: number }[];
}
...
export const addOrUpdateCartItem = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  const user = req.user.id;
  ...
  const cartItem = cart.products.find(
    (p) => p.product.toString() === productID.toString() && p.variant.toString() === variantID.toString()
  );
...
export const deleteAllCartItems = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  const user = req.user.id;
...
export const deleteCartItem = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  const user = req.user.id;
